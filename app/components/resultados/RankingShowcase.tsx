...
export const RankingShowcase: React.FC<RankingShowcaseProps> = ({
  resultados,
}) => {
  const datos: DatosShowcase[] = resultados.showcase
    .map((row) => ({
  ...
  return (
    <div className="card shadow-lg mb-4">
      <div className="card-header bg-success text-white">
        <h3 className="mb-0">
          <Award className="me-2" size={24} style={{ display: "inline" }} />
          Ranking Showcase
        </h3>
      </div>
            ...
            <tbody>
              {datos.map((item, idx) => (
                <tr key={idx}>
                  ...
                  <td>
                    <span className="badge bg-primary">{item.total}/30</span>
                  </td>
                </tr>
              ))}
            </tbody>
